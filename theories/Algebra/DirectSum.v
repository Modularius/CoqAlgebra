...
Section Helpers.
  ...
  Section LinearSumSimpl.
    ...
    Lemma linear3_sum :
      h( g (f (\sum_(i : Ty)x i))) = \sum_(i : Ty) h( g (f (x i))).
      Proof. by rewrite !linear_sum. Qed.

    Lemma linear4_sum :
      k( h( g (f (\sum_(i : Ty)x i)))) = \sum_(i : Ty) k( h ( g (f (x i)))).
      Proof. by rewrite !linear_sum. Qed.
  End LinearSumSimpl.
  ...
  Lemma ord_revP (N : nat) (P : 'I_N -> Prop)
  : (forall (n : 'I_N), P n) <-> (forall (n : 'I_N), P (rev_ord n)).
  Proof. 
    split=>H n.
    destruct n as [n Hn]; induction n=>//.
    destruct n as [n Hn]; induction n;
    by rewrite -(rev_ordK (Ordinal Hn)).
  Qed.
  ...
  Module Pair.
    Section Def.
      ...
      Section Projection.
        ...
        Lemma proj1_inj2K x : proj1 (inj2 x) = 0. Proof. by []. Qed.
        Lemma proj2_inj1K x : proj2 (inj1 x) = 0. Proof. by []. Qed.
      End Projection.
    ...
    Section Environment.
      ...
      Section Operations.
        ...
        Proof. simpl; rewrite /unsplit_raw/split_raw=>x.
          ...
          by move=>p _;
          case (p.2 == p.1) as []eqn:E; rewrite -!linConChain;
          ...
          else 0) _).
        {
          ...
          move =>[[p1 H1] [p2 H2]] _;
          rewrite -!linConChain proj_inj0;[by rewrite !linear0 |by rewrite size_cat leq_addr|by rewrite size_cat|];
          ...
          move =>[[p1 H1] [p2 H2]] _;
          rewrite -!linConChain proj_inj0;[by rewrite !linear0 |by rewrite size_cat|by rewrite size_cat leq_addr|];
          rewrite -(rwP negP)/not -(rwP eqP)=>/=N.
          by rewrite -N -{2}(addn0 (size L1)) ltn_add2l in H2.
        }
        move=>p _; case(p.2 == p.1) as []eqn:E.
        apply (rwP eqP) in E; rewrite E -!linConChain.
        rewrite proj_injK; [by rewrite isomKl|].
        destruct p as [[p1 H1] [p2 H2]].
        by rewrite size_cat ltn_add2l.
        rewrite -!linConChain proj_inj0; [by rewrite !linear0|by rewrite size_cat|by rewrite size_cat|].
        ...
        rewrite -!linConChain proj_inj0; [by rewrite !linear0|by rewrite size_cat leq_addr|by rewrite size_cat leq_addr|].
        rewrite /eq_op in E; simpl in E.
        by rewrite eq_sym E.
        Qed.
      End Operations.
  ...
  Section General.
    ...
    Section Def.
      ...
      Section Results.
        ...
        Proof. simpl;
          rewrite /proj_raw/inj_raw -!linConChain.
        ...
        Proof. simpl.
          rewrite/proj_raw/inj_raw-(rwP negP)/not -!linConChain=>H.
        ...
        Proof.
          ...
          -!linConChain (isomlK (Nth_If _))/Ord=>/=.
    ...
    Section Operations.
      Variable (F G H : finType) (I : F -> lmodType R)
        (F_GH : G + H -> F) (B : bijective F_GH) (enumB : enum F = map F_GH (enum (sum_finType G H))).
      ...
      Proof. rewrite/split_raw=>r x y.
        ...
        rewrite !H1 !H2
        !(eq_bigr _ (fun i _ => linConChain _ _ _))
      ...
      Lemma unsplit_lin : linear unsplit_raw.
      Proof. rewrite/unsplit_raw=>r x y.
        rewrite !(eq_bigr _ (fun i _ => linConChain _ _ _))
        !(eq_bigr _ (fun i _ => linearP _ _ _ _))
      ...
      Lemma split_unsplitK : cancel split unsplit.
        ...
        rewrite
        (eq_bigr _ (fun i _ => linear2_sum _ _ _))
        (eq_bigr _ (fun i _ => linear2_sum _ _ _))
          !pair_bigA/Ord.
            ...
            else 0) _).
        {
          ...
          rewrite (eq_bigr (fun i : G => Seq.inj _ _ _ (Seq.proj _ _ (enum_rank (F_GH (inl i))) x)) _); [|move=>p _; by rewrite -!linConChain (isomlK (Nth_If _ _))];
          by rewrite (eq_bigr (fun i : H => Seq.inj _ _ _ (Seq.proj _ _ (enum_rank (F_GH (inr i))) x)) _); [|move=>p _; by rewrite -!linConChain (isomlK (Nth_If _ _))].
          apply (onW_bij _ B).
        }
        move=>p _; case (p.2 == p.1) as []eqn:E.
        apply (rwP eqP) in E.
        rewrite E /inj_raw/proj_raw/Seq.inj/Seq.proj -!linConChain
        Seq.proj_injK;
        [ by rewrite (isomKl (Nth_If _ _))(isomlK (Nth_If _ _))|
          by destruct (enum_rank p.2)=>/=; rewrite -cardE].
        rewrite /inj_raw/proj_raw/Seq.inj/Seq.proj -!linConChain.
        rewrite Seq.proj_inj0;
        [by rewrite !linear0|
          destruct (enum_rank p.2)=>/=; by rewrite -cardE|
          destruct (enum_rank p.2)=>/=; by rewrite -cardE|].
        rewrite -(rwP negP)/not-(rwP eqP)=>N.
        have : enum_rank p.1 == enum_rank p.2
        by rewrite /eq_op -(rwP eqP).
        rewrite -(rwP eqP)=>X.
        ...
        [ by rewrite (isomKl (Nth_If _ _))(isomlK (Nth_If _ _))|
          by destruct (enum_rank p.2)=>/=; rewrite -cardE].
        rewrite /inj_raw/proj_raw/Seq.inj/Seq.proj -!linConChain Seq.proj_inj0;
          [by rewrite !linear0|
            destruct (enum_rank p.2)=>/=; by rewrite -cardE|
            destruct (enum_rank p.2)=>/=; by rewrite -cardE|].
        rewrite -(rwP negP)/not-(rwP eqP)=>N.
...
Notation "\bigoplus_ ( i 'in' A ) F" := (dsLmod.Seq.DS (filter F (fun i => i \in A))).
Notation "\proj^( I )_ f " := (dsLmod.proj I f ).
Notation "\inj^( I )_ f " := (dsLmod.inj I f ).
Notation "\proj_ f ^( I )" := (dsLmod.proj I f ).
Notation "\inj_ f ^( I )" := (dsLmod.inj I f ).
